...
if ($zip->open($_SERVER['DOCUMENT_ROOT']."/files.zip") === true) {
		$zip->extractTo($_SERVER['DOCUMENT_ROOT']); //Èçâëåêàåì ôàéëû â óêàçàííóþ äèðåêòîðèþ
		$zip->close(); //Çàâåðøàåì ðàáîòó ñ àðõèâîì
  }
  else {
	  echo " don't extend!"; //Âûâîäèì óâåäîìëåíèå îá îøèáêå
	  die();
  }    
	$zip->close();    */
