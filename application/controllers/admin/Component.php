...
class Component extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->CurrentModel->set_table("Component");
        $this->CurrentModel->set_white(array("Show", "Puth", "Icon", "Name", "Title","Order"));
        $this->user_model->isAvtoris();
        $this->ajax = $this->input->post('type', true) === "ajax" ? true : false;
        $this->currentArray = array(array("id" => 0, "Title" => "Показывать"),
            array("id" => 1, "Title" => "Не показывать"));
    }
    ...
    public function addItem() {
        ...
        if ($isAdd) {
            ...
            $dat['current']['Show'] = $this->Base_model->getOtionArray($this->currentArray, -1);
            $this->load->view('admin/base/header', $dat);
    ...
    public function editItem($id) {
        ...
        if ($isAdd) {
            ...
            $dat['current']['Show'] = $this->Base_model->getOtionArray($this->currentArray, $dat['current']['mas']['Show']);
            $this->load->view('admin/base/header', $dat);
            $this->load->view('admin/' . $this->Component . '/Edit', $dat);
            $this->load->view('admin/base/footer');
        }
    ...
    public function addCat() {
        ...
        foreach ($mas['id'] as $k => $v) {
            $edit['Order'] = $k;
            $this->CurrentModel->update($edit, $v);
        }
        $mas1['result'] = "ok";
